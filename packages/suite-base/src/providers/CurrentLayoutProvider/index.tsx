...
export default function CurrentLayoutProvider({
  ...
  useAsync(async () => {
    ...
    const targetDefaultLayoutId = "new_default_layout_id" as LayoutID;
    const flyfishDefaultLayout = await layoutManager.getLayout(targetDefaultLayoutId)
    if (!flyfishDefaultLayout) {
      await layoutManager.saveNewLayout({
        id: targetDefaultLayoutId,
        name: "Default Flyfish Layout",
        data: defaultFlyfishLayout,
        permission: "CREATOR_WRITE",
      });
    }
    await setSelectedLayoutId(targetDefaultLayoutId, { saveToProfile: false });
